...
- 🎯 Entry points to execute your program using `python -m <manoreza>` or `$ manoreza` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/masoodkamyab/manoreza/branch/main/graph/badge.svg?token=manoreza_token_here)](https://codecov.io/gh/masoodkamyab/manoreza)
[![CI](https://github.com/masoodkamyab/manoreza/actions/workflows/main.yml/badge.svg)](https://github.com/masoodkamyab/manoreza/actions/workflows/main.yml)

Awesome manoreza created by masoodkamyab

## Install

from source
```bash
git clone https://github.com/masoodkamyab/manoreza manoreza
cd manoreza
make install
```
...
```bash
pip install manoreza
```

## Executing

```bash
$ manoreza run --port 8080
```

or

```bash
python -m manoreza run --port 8080
```

or

```bash
$ uvicorn manoreza:app
```
...
```bash
❯ manoreza --help
Usage: manoreza [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ manoreza create-user --help
Usage: manoreza create-user [OPTIONS] USERNAME PASSWORD
...
```bash
manoreza create-user admin admin --superuser
```
...
Run with `manoreza run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `manoreza create-user`.
...
-----------------------------------------------------
manoreza/__init__.py              4      0   100%
manoreza/app.py                  16      1    94%
manoreza/cli.py                  21      0   100%
manoreza/config.py                5      0   100%
manoreza/db.py                   10      0   100%
manoreza/models/__init__.py       0      0   100%
manoreza/models/content.py       47      1    98%
manoreza/routes/__init__.py      11      0   100%
manoreza/routes/content.py       52     25    52%
manoreza/routes/security.py      15      1    93%
manoreza/routes/user.py          52     26    50%
manoreza/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from manoreza.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in manoreza/default.toml.

### As environment variables
```bash
export manoreza_KEY=value
export manoreza_KEY="@int 42"
export manoreza_KEY="@jinja {{ this.db.uri }}"
export manoreza_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
manoreza_ENV=production manoreza run
```
